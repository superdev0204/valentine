...
<div class="container">
    ...
    <form method="POST" 
          action="{{ isset($fedexMapping) 
              ? route('admin.fedex_mappings.school_outgoing.update', $fedexMapping) 
              : route('admin.fedex_mappings.school_outgoing.store') }}">
        @csrf
        ...
        <div class="mb-3">
            ...
            <select name="our_field" class="form-select">
                ...
                <optgroup label="Schools Table">
                    @foreach($allFields['schools'] as $field)
                        <option value="{{ $field }}"
                ...
                <optgroup label="Box Size Matrices Table">
                    @foreach($allFields['school_box_size_matrices'] as $field)
                        <option value="{{ $field }}"
                            {{ old('our_field', $fedexMapping->our_field ?? '') == $field ? 'selected' : '' }}>
                            {{ $field }}
                        </option>
        ...
        <a href="{{ route('admin.fedex_mappings.school_outgoing') }}" class="btn btn-secondary">Cancel</a>
    </form>
</div>
@endsection
